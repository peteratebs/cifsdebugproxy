...
static int http_server_demo_restart(void)
{
	...
	*/
	...
	if (HTTP_ServerAddPostHandler(&ExampleServerCtx.httpServer, "\\smbdiag_ajax_getmessages", ajax_update_demo_cb, (void *)"get_value") < 0)
		return -1;
	rtp_printf("Post Handlers for smbdiag_ajax_getmessages has been assigned\n");
	if (HTTP_ServerAddPostHandler(&ExampleServerCtx.httpServer, "\\smbdiag_ajax_getfids", ajax_update_demo_cb, (void *)"get_value") < 0)
		return -1;
	rtp_printf("Post Handlers for smbdiag_ajax_getfids has been assigned\n");
	return (0);
}
...
HTTP_CHAR cgiArgs[256];
    ...
    else if (rtp_strcmp(request->target, "/smbdiag_ajax_getmessages")==0)
    {
    int l,j;
        {
          diag_send_command("SMB MSSGS");
          j = diag_recv_results_from_smb(respBuffer, respBufferSize);
          if (j == 0)
            strcpy(respBuffer,"Failed retrieving a response from the server");
        }
