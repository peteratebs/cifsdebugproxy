...
int rtp_net_getsockname (RTP_HANDLE sockHandle, unsigned char *ipAddr, int *port, int *type)
{
    ...
    if (getsockname ((int) sockHandle, (struct sockaddr *)&localAddr, (int *)&localLen) != 0)
    {
#ifdef RTP_DEBUG
                     ...
                     unsigned char * ipAddr, int port, int type)
{
    ...
    /* ----------------------------------- */
    errno = 0;
    result = (long) sendto((int) sockHandle, (const char *) buffer, (int) size, 0, res->ai_addr, res->ai_addrlen);
    freeaddrinfo(res);
                       ...
                       unsigned char *ipAddr, int *port, int *type)
{
    ...
    result = (long) recvfrom((int) sockHandle, (char *) buffer, (int) size, 0, (struct sockaddr *) &remote, (int *) &remoteLen);
